...
DEFAULT_AD_TEXT = os.getenv("DEFAULT_AD_TEXT", "Рекламируйтесь здесь с KYRON Ads").strip()
...
def premium_icon(name: str, fallback: str = "") -> str:
    ...
    if raw:
        return raw
    return PREMIUM_EMOJI_DEFAULTS.get(name, fallback) or fallback
...
DEFAULT_SETTINGS = {
    ...
    "strength_on": True,
    "strength_emoji": '<tg-emoji emoji-id="5265173719239643202">✅</tg-emoji>',
...
def is_stale_buy_ts(ts: Any, max_age_sec: int = OLD_BUY_MAX_AGE_SEC) -> bool:
    try:
    ...
    if ts_i > 10_000_000_000:
        ts_i //= 1000
    now = int(time.time())
    return ts_i < (now - max_age_sec)

def effective_ignore_before_ts(token: Dict[str, Any]) -> int:
    """Ignore history older than the later of per-token baseline and process start grace window.
    This prevents replaying hours-old buys after restart/redeploy even when remote APIs return them as "new".
    """
    try:
        token_ts = int((token or {}).get("ignore_before_ts") or 0)
    except Exception:
        token_ts = 0
    startup_ts = int(PROCESS_START_TS - STARTUP_HISTORY_GRACE_SEC)
    return max(token_ts, startup_ts)
...
def _stonfi_extract_swaps_from_actions(actions: Any, token_addr: str, tx_hash: str = "") -> List[Dict[str, Any]]:
    """Very strict STON swap parser from TonAPI action lists.

    It only trusts actions where the input and output assets are both explicit.
    Result items include direction='buy' for TON->token and direction='sell' for token->TON.
    Ambiguous actions are skipped.
    """
    out: List[Dict[str, Any]] = []
    if not isinstance(actions, list):
        return out

    token_addr_s = _norm_addr(token_addr)
    TON_SYMS = {"ton", "wton", "pton"}

    def _asset_addr(x: Any) -> str:
        if isinstance(x, dict):
            for k in ("address", "master", "jetton_master", "jettonMaster", "token_address", "tokenAddress", "asset_address", "assetAddress"):
                v = x.get(k)
                if v:
                    return _norm_addr(v)
        return ""

    def _asset_sym(x: Any) -> str:
        if not isinstance(x, dict):
            return ""
        return str(x.get("symbol") or x.get("ticker") or x.get("name") or x.get("asset") or "").strip().lower()
    ...
    def _is_ton_asset(x: Any) -> bool:
        ...
        if native is True:
            return True
        addr = _asset_addr(x)
        return False if addr else False

    def _norm_asset(v: Any) -> Dict[str, Any]:
        return v if isinstance(v, dict) else {}

    def _pick_asset(aa: Dict[str, Any], side: str) -> Dict[str, Any]:
        if side == 'in':
            keys = (
                "asset_in", "assetIn", "in", "in_asset", "from_asset", "fromAsset",
                "offer_asset", "offerAsset", "source_asset", "sourceAsset", "src_asset"
            )
        else:
            keys = (
                "asset_out", "assetOut", "out", "out_asset", "to_asset", "toAsset",
                "ask_asset", "askAsset", "destination_asset", "destinationAsset", "dst_asset"
            )
        for k in keys:
            v = aa.get(k)
            if isinstance(v, dict) and v:
                return v
        swap = aa.get("swap") if isinstance(aa.get("swap"), dict) else {}
        for k in keys:
            v = swap.get(k)
            if isinstance(v, dict) and v:
                return v
    ...
    def _pick_amount(aa: Dict[str, Any], side: str, asset: Any) -> Optional[float]:
        if side == 'in':
            keys = ("amount_in", "amountIn", "in_amount", "amount", "offer_amount", "offerAmount", "from_amount", "fromAmount")
        else:
            keys = ("amount_out", "amountOut", "out_amount", "ask_amount", "askAmount", "to_amount", "toAmount")
        raw = None
        for k in keys:
    ...
    for a in actions:
        ...
        if isinstance(payload, dict):
            aa.update(payload)
        swap = aa.get('swap') if isinstance(aa.get('swap'), dict) else None
        if swap:
            aa.update(swap)
        ...
        in_asset = _norm_asset(_pick_asset(aa, 'in'))
        out_asset = _norm_asset(_pick_asset(aa, 'out'))
        if not in_asset or not out_asset:
            continue
        ...
        # Require one side to be explicit token and the other to be explicit TON-like.
        if in_is_token and out_is_ton:
            direction = 'sell'
        elif in_is_ton and out_is_token:
            direction = 'buy'
        else:
            continue

        amt_in = _pick_amount(aa, 'in', in_asset)
        amt_out = _pick_amount(aa, 'out', out_asset)
        if not amt_in or not amt_out:
            continue
        ...
        out.append({
            ...
            "ton": amt_in if direction == 'buy' else amt_out,
            "token_amount": amt_out if direction == 'buy' else amt_in,
            "direction": direction,
        })
    return out


def _stonfi_event_has_explicit_sell(ev: Dict[str, Any], token_addr: str) -> bool:
    try:
        swaps = _stonfi_extract_swaps_from_actions((ev or {}).get("actions"), token_addr, tonapi_event_tx_hash(ev))
        return any(str(x.get('direction')) == 'sell' for x in swaps)
    except Exception:
        return False


def _stonfi_tx_has_explicit_sell(tx: Dict[str, Any], token_addr: str) -> bool:
    try:
        swaps = _stonfi_extract_swaps_from_actions((tx or {}).get("actions"), token_addr, _tx_hash(tx))
        return any(str(x.get('direction')) == 'sell' for x in swaps)
    except Exception:
        return False
...
def stonfi_extract_buys_from_tonapi_tx(tx: Dict[str, Any], token_addr: str) -> List[Dict[str, Any]]:
    """Strict STON buy parser from TonAPI tx actions. BUY = TON -> token_addr."""
    return [x for x in _stonfi_extract_swaps_from_actions(tx.get("actions"), token_addr, _tx_hash(tx)) if str(x.get("direction")) == 'buy']
...
def stonfi_extract_buys_from_tonapi_event(ev: Dict[str, Any], token_addr: str) -> List[Dict[str, Any]]:
    ...
    return [x for x in _stonfi_extract_swaps_from_actions((ev or {}).get("actions"), token_addr, tonapi_event_tx_hash(ev)) if str(x.get("direction")) == 'buy']
...
def _discover_token_setup_sync(jetton: str, dex_mode: str = "both", sym_hint: str = "") -> Dict[str, Any]:
    """Resolve token metadata and pools synchronously.

    Run this inside asyncio.to_thread so Telegram handlers stay responsive.
    """
    name = ""
    sym = ""
    ...
    lp_name, lp_watches = launchpad_maps_for_token(jetton)
    extra_holders = []
    try:
        extra_holders = tonapi_jetton_holder_addresses(jetton, 8)
    except Exception:
        extra_holders = []
    ...
    return {
        ...
        "launchpad": (lp_name if lp_name else ""),
        "launchpad_watch": (lp_watches[0] if lp_watches else ""),
        "launchpad_watch_addresses": (lp_watches + extra_holders),
        "blum_mode": bool(lp_name in ("blum", "gaspump", "groypi", "groypad")) or bool(lp_watches),
...
async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    processing_msg = await update.message.reply_text("⏳ Adding token... Please wait.")
    dm = (dex_mode or "both").lower().strip()
    setup = await _to_thread(_discover_token_setup_sync, jetton, dm, sym_hint)
    jetton = str(setup.get("address") or jetton)
    name = str(setup.get("name") or "").strip()
    sym = str(setup.get("symbol") or "").strip()
    holders_seed = setup.get("holders")
    decimals_seed = int(setup.get("decimals") or 9)
    ston_pool = setup.get("ston_pool")
    dedust_pool = setup.get("dedust_pool")
    ...
    tok = {
        ...
        "ston_pool": ston_pool,
        "dedust_pool": dedust_pool,
        "blum_mode": bool(setup.get("blum_mode")),
        "launchpad": str(setup.get("launchpad") or ""),
        "launchpad_watch": str(setup.get("launchpad_watch") or ""),
        "launchpad_watch_addresses": list(setup.get("launchpad_watch_addresses") or []),
        "launchpad_opcode": "",
    ...
    try:
        await _to_thread(lambda: refresh_launchpad_config(tok, force=True))
...
async def _set_token_now(chat_id: int, jetton: str, context: ContextTypes.DEFAULT_TYPE, reply_chat_id: int, telegram: str = "", dex_mode: str = "both", announce: bool = True):
    processing_msg = None
    try:
        processing_msg = await context.bot.send_message(chat_id=reply_chat_id, text="⏳ Processing token... Please wait.")
    except Exception:
        processing_msg = None

    setup = await _to_thread(_discover_token_setup_sync, jetton, dex_mode, "")
    jetton = str(setup.get("address") or jetton)
    name = str(setup.get("name") or "Token")
    sym = str(setup.get("symbol") or (name[:10] or "TOKEN").upper())
...
async def poll_once(app: Application):
    ...
    for chat_id, g in items:
        ...
        if settings.get("enable_ston", True) and token.get("ston_pool"):
            ...
            try:
                # Fast path first: check the latest pool transactions before events.
                    ...
                    if isinstance(txs_fast, list) and txs_fast:
                        ...
                        for txo in reversed(txs_fast):
                            ...
                            for b in buys:
                                ...
                                dedupe_key = ('tx:' + _normalize_tx_hash_to_hex(txh)) if txh else ('ston-tx:' + str(pool) + ':' + str(ut))
                    ...
                    if isinstance(events, list) and events:
                        ...
                        if not last_eid and not last_ets:
                            ...
                            for ev in reversed(new_events):
                                if _stonfi_event_has_explicit_sell(ev, token['address']):
                                    continue
...
async def post_buy(app: Application, chat_id: int, token: Dict[str, Any], b: Dict[str, Any], source: str):
    ...
    def build_group_message() -> str:
        ...
        if tok_amt and tok_symbol:
            ...
            try:
                tok_amt_f = float(tok_amt)
                token_line = f'{premium_text_or_plain("got", "↔️")} <b>{h(fmt_token_amount(tok_amt_f))} {sym_html}</b>'
            except Exception:
        ...
        holders_line_local = f'{premium_text_or_plain("holders", "🫂")} {h(holders_compact)} Holders'
        ...
        buyer_line_local = f'{premium_text_or_plain("wallet", "🫂")} {buyer_html_local}{pct_part}{txn_part}'

        price_line_local = f'{premium_text_or_plain("price", "🗝️")} Price: —'
        if price_usd is not None:
            try:
                price_line_local = f'{premium_text_or_plain("price", "🗝️")} Price: ${float(price_usd):,.6f}'
            except Exception:
                price_line_local = f'{premium_text_or_plain("price", "🗝️")} Price: —'

        mcap_line_local = f'{premium_text_or_plain("mcap", "📈")} MarketCap: {h(fmt_usd(mc_usd, 0) or "—")}'
        ...
        chart_part_local = f'{premium_text_or_plain("chart", "🛡️")} <a href="{h(chart_link)}">Chart</a>' if chart_link else f'{premium_text_or_plain("chart", "🛡️")} Chart'
        links_row_local = " | ".join([p for p in [listing_part, trade_part, chart_part_local] if p])
        ...
        blocks.append(mcap_line_local)
        blocks.append("")
        blocks.append(links_row_local)
    ...
    def build_trending_channel_message() -> str:
        ...
        links_row = " | ".join([p for p in [listing_part, trade_part, chart_part] if p])
    ...
    def build_buy_keyboard(dest_chat_id: int) -> InlineKeyboardMarkup:
        ...
        btns = []
        if trending:
            btns.append(InlineKeyboardButton("Trending", url=trending))
        if buy_url:
            btns.append(InlineKeyboardButton("DTrade", url=buy_url))
        if btns:
            rows.append(btns)
...
FORCED_CHANNEL_CUSTOM_EMOJI_ID = "5265173719239643202"
...
def main():
    ...
    application = ApplicationBuilder().token(BOT_TOKEN).post_init(post_init).concurrent_updates(True).build()
