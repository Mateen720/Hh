...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "CONFIRM_PREVIEW":
        ...
        try:
            await q.edit_message_text("⏳ Confirmed. Adding token now…")
        except Exception:
            pass
...
async def post_buy(app: Application, chat_id: int, token: Dict[str, Any], b: Dict[str, Any], source: str):
    ...
    def channel_premium_strength_line(ton_amount: float) -> str:
        """Premium emoji strength row for channel style only."""
        try:
            step = float(s.get("strength_step_ton") or 5.0)
            max_n = int(s.get("strength_max") or 30)
            n = 1 if float(ton_amount) > 0 else 0
            if step > 0 and float(ton_amount) > 0:
                effective_step = max(step, float(ton_amount) / 9.0)
                n = max(1, int(float(ton_amount) // effective_step))
            n = min(max_n, max(1, n))
            return repeat_emoji(CHANNEL_PREMIUM_BAR_EMOJI, n)
        except Exception:
            return repeat_emoji(CHANNEL_PREMIUM_BAR_EMOJI, 8)
    ...
    def build_trending_channel_message() -> str:
        ...
        # Premium emoji strength line for channel style only.
        checks = channel_premium_strength_line(ton_amt)
        ...
        if tok_amt and tok_symbol:
            ...
            try:
                tok_amt_f = float(tok_amt)
                token_line = f'{CHANNEL_PREMIUM_BUY_EMOJI} <b>{h(fmt_token_amount(tok_amt_f))} {sym_html}</b>'
            except Exception:
                token_line = f'{CHANNEL_PREMIUM_BUY_EMOJI} <b>{h(tok_amt)} {sym_html}</b>'
        ...
        holders_line_ch = f"{CHANNEL_PREMIUM_HOLDERS_EMOJI} {h(holders_compact)} Holders"
        ...
        # Price + MarketCap
        price_line = f"{CHANNEL_PREMIUM_PRICE_EMOJI} Price: —"
        if price_usd is not None:
            try:
                price_line = f"{CHANNEL_PREMIUM_PRICE_EMOJI} Price: ${float(price_usd):,.6f}"
            except Exception:
                price_line = f"{CHANNEL_PREMIUM_PRICE_EMOJI} Price: —"
        mc_line_ch = f"{CHANNEL_PREMIUM_MCAP_EMOJI} MarketCap: {h(fmt_usd(mc_usd, 0) or '—')}"
...
# Premium emoji set imported from the reference design file.
# Applied only to channel / trending style, never to group style.
CHANNEL_PREMIUM_BAR_EMOJI = '<tg-emoji emoji-id="5422508107982973442">🚀</tg-emoji>'
CHANNEL_PREMIUM_BUY_EMOJI = '<tg-emoji emoji-id="5422727112660364808">🚀</tg-emoji>'
CHANNEL_PREMIUM_HOLDERS_EMOJI = '<tg-emoji emoji-id="5420565528634730477">🚀</tg-emoji>'
CHANNEL_PREMIUM_PRICE_EMOJI = '<tg-emoji emoji-id="5895612228949776244">🚀</tg-emoji>'
CHANNEL_PREMIUM_MCAP_EMOJI = '<tg-emoji emoji-id="5785162321684860622">🚀</tg-emoji>'
